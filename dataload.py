...
class DATA_LOADER(object):
    def __init__(self, opt):
        ...
        # self.tr_cls_centroid = np.zeros([self.seenclasses.shape[0], self.feature_dim], np.float32) # 横着的
        # for i in range(self.seenclasses.shape[0]):
    ...
    def read_matimagenet(self, opt):
        if opt.preprocessing:
            ...
            image_path = opt.dataroot + "/" + opt.dataset + "/" + opt.image_embedding + ".mat"
            feat_matcontent = h5py.File(image_path, 'r')    #list(matcontent.keys()) == ['#refs#', 'features', 'features_val', 'image_files', 'image_files_val', 'labels', 'labels_val']
            feature = feat_matcontent['features'][()]  # (2048, 1281167)
            feature = feature.T  # (1281167, 2048)
            features_val = feat_matcontent['features_val'][()] # (50000, 2048)
            label = feat_matcontent['labels'][()] #(1, 1281167)
            labels_val = feat_matcontent['labels_val'][()] #(1, 50000)
            feature = scaler.fit_transform(feature)
            label = label.astype(int).squeeze() - 1
            feature_val = scaler.transform(features_val)
            label_val = labels_val.astype(int).squeeze() - 1
            feat_matcontent.close()
            
            matcontent = h5py.File('../data/ImageNet/extract_res/res101_1crop_2hops_t.mat', 'r')
        ...
        matcontent = sio.loadmat(opt.dataroot + "/" + opt.dataset + "/" + opt.class_embedding + ".mat") #list(matcontent.keys()) == ['__header__', '__version__', '__globals__', 'all', 'hops2', 'hops3', 'lp1000', 'lp500', 'lp5000', 'mp1000', 'mp500', 'mp5000']
        w2v_matcontent = h5py.File(opt.dataroot + "/" + opt.dataset + "/" + opt.class_embedding + ".mat")#['#refs#', 'no_w2v_loc', 'w2v', 'wnids', 'words']
    ...
    def read_matdataset(self, opt):
        ...
        feature = matcontent['features'].T  # 11788 x 2048
        ...
        self.attribute = torch.from_numpy(matcontent['att'].T).float()  # 200 x 312
        if not opt.validation:
            if opt.preprocessing:
        ...
        self.seenclasses = torch.from_numpy(np.unique(self.train_label.numpy()))
        self.unseenclasses = torch.from_numpy(np.unique(self.test_unseen_label.numpy()))
        self.unseenattributes = self.attribute[self.unseenclasses] # 50 x 312
        self.ntrain = self.train_feature.size()[0]
        self.ntrain_class = self.seenclasses.size(0)
        self.ntest_class = self.unseenclasses.size(0)
