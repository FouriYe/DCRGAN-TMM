...
import argparse
import classifier
import classifier2forSR    #for searched repre
import model
import soft_cls
import util
import torch
...
if opt.gzsl:
    log_acc_name = out_subdir + '/log_DCRGAN_acc_GZSL_{:s}_'.format(exp_info)
else:
    log_acc_name = out_subdir + '/log_DCRGAN_acc_ZSL_{:s}_'.format(exp_info)
...
def save_Separated_model(opt,competitor,best_acc):
    if opt.gzsl:
        competitor_acc=competitor.H_ensemble2
        competitor_seen_acc = competitor.seen_ensemble2
        competitor_unseen_acc = competitor.unseen_ensemble2
        if competitor_acc>best_acc:
            best_acc = competitor_acc
            files2removeGZSL = glob.glob(out_subdir + '/'+str(opt.rec_attSize)+'_Best_model_GZSL_*')
            for _i in files2removeGZSL:
                os.remove(_i)
            torch.save({'state_dict_G': netG.state_dict(),'state_dict_GZSL_classifier': competitor.model.state_dict()}, out_subdir + '/'+str(opt.rec_attSize)+'_Best_model_GZSL_H_{:.2f}_S_{:.2f}_U_{:.2f}.tar'.format(competitor_acc*100,competitor_seen_acc*100,competitor_unseen_acc*100))
        return best_acc
    else:
        competitor_acc=competitor.ensemble_acc2
        if competitor_acc>best_acc:
            best_acc = competitor_acc
...
def calc_gradient_penalty_withSR(netD, SRN, real_data, fake_data, input_att):
    ...
    disc_interpolates = netD(interpolates, Variable(input_att), input_sr)
...
def train_D():
    ...
    criticD_real = netD(input_resv, input_attv, input_sr.detach())
    ...
    criticD_fake = netD(fake.detach(), input_attv, input_sr.detach())
    criticD_fake = criticD_fake.mean()
    criticD_fake.backward(one)
    # gradient penalty
    gradient_penalty = calc_gradient_penalty_withSR(netD, SRN, input_res, fake.data, input_att)
...
def train_G():
    ...
    criticG_fake = netD(fake, input_attv, input_sr)
    ...
    errG = G_cost + opt.cyc_seen_weight * r_errG_seen + opt.cyc_unseen_weight * r_errG_unseen + opt.cyc_seen_weight2 * r_errG_seen2 + opt.cyc_unseen_weight2 * r_errG_unseen2
    errG.backward()
...
def val_ZSL(syn_unseen_feature, syn_unseen_label, best_acc, log_acc_log):
    ...
    #print log
    log_text = 'Visual Softmax: {:.2f}%'.format(cls.cls_acc*100)
    print(log_text)
    log_acc_log.write(log_text+'\n')
    
    log_text = 'V2S Softmax   : {:.2f}%'.format(v2s.acc*100)
    print(log_text)
    log_acc_log.write(log_text+'\n')
    
    log_text = 'V2SR Softmax   : {:.2f}%'.format(v2sr.acc*100)
    print(log_text)
    log_acc_log.write(log_text+'\n')

    log_text = 'ZSL Separated Ensemble (V+SR): {:.2f}%'.format(cls.ensemble_acc*100)  #note: if use SR, ensemble_acc is visual with searched repre
    print(log_text)
    log_acc_log.write(log_text+'\n')
    
    log_text = 'ZSL Separated Ensemble (V+SR+S):  {:.2f}%; bestAcc: {:.2f}%'.format(cls.ensemble_acc2*100, best_acc*100)
    print(log_text)
    log_acc_log.write(log_text+'\n')
...
def val_GZSL(syn_unseen_feature, syn_unseen_label, best_acc, log_acc_log):
    ...
    v2s = soft_cls.Visual_to_semantic(opt,log_acc_log, netS(train_X_var).data.cpu(), train_Y,data, nclass, generalized=True)
    opt.gzsl_unseen_outputS = v2s.unseen_out
    opt.gzsl_seen_outputS = v2s.seen_out
    
    v2sr = soft_cls.Visual_to_RecSemantic(opt,log_acc_log, netS2(train_X_var).data.cpu(), train_Y,data, nclass, opt.softRS_lr, generalized=True)
    opt.gzsl_unseen_outputRS = v2sr.unseen_out
    opt.gzsl_seen_outputRS = v2sr.seen_out
    
    cls = classifier2forSR.CLASSIFIER(opt, train_X_var, train_Y, data, nclass, _beta1=0.5, _nepoch=25, generalized=True)
    #save models
    best_acc = save_Separated_model(opt,cls,best_acc)
    
    #print log
    log_text = 'GZSL Visual Softmax: Seen Acc: {:.2f}%, Unseen Acc: {:.2f}%, H Acc: {:.2f}%'.format(cls.seen_cls * 100,cls.unseen_cls * 100,cls.H_cls * 100)
    print(log_text)
    log_acc_log.write(log_text+'\n')
    
    log_text = 'V2S Softmax: Seen Acc: {:.2f}%, Unseen Acc: {:.2f}%, H Acc: {:.2f}%'.format(v2s.acc_seen * 100,v2s.acc_unseen * 100,v2s.H * 100)
    print(log_text)
    log_acc_log.write(log_text+'\n')
    

    log_text = 'V2SR Softmax: Seen Acc: {:.2f}%, Unseen Acc:{:.2f}%, H Acc:{:.2f}%'.format(v2sr.acc_seen * 100,v2sr.acc_unseen * 100,v2sr.H * 100)
    print(log_text)
    log_acc_log.write(log_text+'\n')

    log_text = 'GZSL Separated Ensemble (V+SR): Seen Acc : {:.2f}%, Unseen Acc: {:.2f}%, H Acc: {:.2f}%'.format(cls.seen_ensemble * 100,cls.unseen_ensemble * 100,cls.H_ensemble * 100)  #note: if use SR, ensemble_acc is visual with searched repre
    print(log_text)
    log_acc_log.write(log_text+'\n')
    
    log_text = 'GZSL Separated Ensemble (V+SR+S): Seen Acc : {:.2f}%, Unseen Acc: {:.2f}%, H Acc: {:.2f}%; bestAcc: {:.2f}%'.format(cls.seen_ensemble2 * 100,cls.unseen_ensemble2 * 100,cls.H_ensemble2 * 100, best_acc*100)
    print(log_text)
    log_acc_log.write(log_text+'\n')
    return best_acc
